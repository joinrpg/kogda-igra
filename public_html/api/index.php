<?php
  ...
  выдается ответ вида <code>{"id":"<i>(id)</i>", "access-denied":"1"}</code>. </li>
    <li>Если <code>id</code> не верный или игра ожидает модерации, выдается пустой объект.</li>
  ...
  Выдает все обновленные игры с <code>timestamp</code> (UNIX timestamp). Игры, ожидающие модерации, не включаются в выдачу.
